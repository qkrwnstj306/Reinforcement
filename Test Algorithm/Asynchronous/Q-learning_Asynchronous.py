...
def train(global_model, target_model, rank):
    ...
    for n_epi in range(max_train_ep):
        ...
        while not done:
            ...
            _max_q_prime = target_model(torch.tensor(s_prime_lst, dtype=torch.float)).max(dim=1, keepdim=True) #global 이 아니라 fixed target network
            max_q_prime_lst = _max_q_prime[0]
            target = r_lst + gamma * max_q_prime_lst * done_mask
        ...
        if n_epi % target_update_interval==0 and rank == 1: # target network 는 global 이라 한 번만 update
